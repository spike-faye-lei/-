...
边界说明（仅用于本地功能演示，不代表完整合规方案）：
1. 只采集用户在公开平台【自愿公开发布】的信息（V2EX 酷工作帖子 / Gitee 公开仓库），限速 1 秒/请求，不采集联系方式等敏感字段
2. 走平台公开 API，不绕过任何反爬机制
3. 演示环境不落地个人敏感信息、不对外提供服务、不用于真实招聘决策
4. 生产环境必须：接入平台官方授权接口（BOSS 开放平台 / 猎聘 API 等）+ 取得候选人单独授权同意 + 加密存储与可审计删除机制
