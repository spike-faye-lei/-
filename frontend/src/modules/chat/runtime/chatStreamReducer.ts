...
export type ChatStreamAction =
  ...
  | { type: 'message_complete'; keyboard?: Array<{ id: string; label: string; type: string; icon?: string; style?: string }> }
  | { type: 'stop_streaming' }
  | { type: 'clear_messages' }
...
export function mapRealtimeMessageToAction(
  ...
  switch (message.type) {
    ...
    case 'message_complete':
      return { type: 'message_complete', keyboard: message.keyboard }
    default:
...
export function chatStreamReducer(
  ...
  switch (action.type) {
    ...
    case 'message_complete':
      return reduceMessageComplete(state, action)
    case 'stop_streaming':
...
function reduceMessageComplete(state: ChatStreamRuntimeState, action: ChatStreamAction): ChatStreamRuntimeState {
  const messageIndex = findMessageIndex(state.messages, state.currentStreamingMessageId)
  let nextMessages = state.messages

  if (messageIndex !== -1) {
    const updates: Partial<ChatMessage> = {}
    if (state.messages[messageIndex].isThinking) {
      updates.isThinking = false
      updates.content = state.messages[messageIndex].content || ''
    }
    if ('keyboard' in action && action.keyboard) {
      updates.keyboard = action.keyboard as ChatMessage['keyboard']
    }
