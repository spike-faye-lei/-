...
const handleQuickAction = async (action: { id: string; label: string; type: string; icon?: string; style?: string }) => {
  if (!action?.type) return

  if (action.type.startsWith('view:')) {
    const slug = action.type.slice(5)
    window.open(`/wiki/read/${slug}`, '_blank')
    return
  }

  if (action.type.startsWith('open_url:')) {
    const url = action.type.slice(9)
    window.open(url, '_blank')
    return
  }

  if (action.type === 'open:wiki') {
    router.push('/wiki')
    return
  }

  if (action.type.startsWith('save_kb:')) {
    router.push('/wiki')
    return
  }
  ...
  if (action.type.startsWith('search:') || action.type.startsWith('web_search:')) {
    const query = action.type.includes(':') ? action.type.split(':')[1] || action.label : action.label
...
const triggerFileUpload = () => {
  ...
  input.type = 'file'
  input.accept = '.pdf,.docx,.md,.txt,.py,.json,.yaml,.yml,.js,.ts,.html,.css'
