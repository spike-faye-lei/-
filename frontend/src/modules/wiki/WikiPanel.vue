        ...
        <div class="filter-controls">
          ...
          <div class="search-box">
            ...
            <button
              ...
              @click="searchQuery = ''; askResult = null"
            >
              清除
            </button>
            <button
          ...
          <!-- AI 问答结果 -->
          <div v-if="askResult" class="ask-result-card">
            <div class="ask-card-header">
              <span class="ask-badge" :class="{ web: askFromWeb }">{{ askFromWeb ? '🌐 联网' : '📚 本地' }}</span>
              <span class="ask-time">刚刚</span>
            </div>
            <div class="ask-answer" v-html="askAnswerHtml" />
            <a v-if="entryUrl" :href="entryUrl" target="_blank" class="entry-open-link">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
              查看完整文章
            </a>
            <div v-if="askSources.length" class="ask-sources">
              <p class="sources-title">{{ askFromWeb ? '参考来源' : '来自知识库' }}</p>
              <a v-for="(s, i) in askSources" :key="i" :href="s.url" target="_blank" class="source-link">
          ...
          <!-- 问答加载中 -->
          <div v-if="asking" class="ask-loading">
            <span class="ask-loading-dot" />
            <span class="ask-loading-text">正在搜索知识库{{ askFromWeb ? '和网络' : '' }}...</span>
          </div>
        </div>
...
const askResult = ref<string | null>(null)
const askSources = ref<{ title: string; url: string }[]>([])
const askFromWeb = ref(false)
const entryUrl = ref('')

const askAnswerHtml = computed(() => {
  if (!askResult.value) return ''
  return askResult.value.replace(/\n/g, '<br>').replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
...
async function handleAsk() {
  if (!searchQuery.value.trim() || asking.value) return
  asking.value = true
  try {
    ...
    askResult.value = data.answer || '未找到结果'
    askSources.value = data.sources || []
    askFromWeb.value = data.from_web || false
    entryUrl.value = data.entry_url || ''
  } catch {
    askResult.value = '搜索失败，请稍后重试'
    askSources.value = []
  } finally {
    asking.value = false
  }
