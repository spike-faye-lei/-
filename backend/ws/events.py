...
def _auto_link_wiki_titles(text: str, wiki_dir=None) -> str:
    """扫描 AI 回复文本，自动将匹配的词条标题转为 [[slug]] 链接。

    Args:
        text: AI 回复文本
        wiki_dir: wiki concepts 目录路径 (Path or None)

    Returns:
        已插入 [[slug]] 链接的文本
    """
    if not text or not wiki_dir:
        return text
    try:
        from pathlib import Path
        import yaml
            ...
            if content.startswith("---"):
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        pass
        # 在文本中查找并替换词条标题（跳过已有 [[...]] 的）
        import re
        for title, slug in sorted(title_to_slug.items(), key=lambda x: -len(x[0])):
            pattern = re.compile(rf"(?<!\[\[)(?<!\[)({re.escape(title)})(?!\]\])(?!\])")
            text = pattern.sub(f"[[{slug}]]", text)
    except Exception:
        pass
    return text
...
async def handle_message_event(
    ...
    try:
        ...
        # 自动为 AI 回复中的词条标题插入 [[slug]] 链接
        try:
            workspace = getattr(agent_loop, "workspace", None)
            wiki_dir = workspace / "wiki" / "concepts" if workspace else None
            assistant_content = _auto_link_wiki_titles(assistant_content, wiki_dir)
        except Exception:
            pass
        ...
        if persisted_content:
            ...
            keyboard = None
            try:
                from backend.modules.tools.quick_actions import build_quick_actions
                ...
                try:
                    ...
                    workspace = getattr(agent_loop, "workspace", None)
                    wiki_dir = Path("workspace/wiki/concepts") if not workspace else workspace / "wiki" / "concepts"
                    wiki_service = WikiService(wiki_dir=wiki_dir)
                    wiki_hits = await wiki_service.hybrid_search(query=content, top_k=5)
                    if len(wiki_hits) < 2:
                        web_results = await web_search(content)
                except Exception:
                    pass

                keyboard = build_quick_actions(
            ...
            # 发送完成通知
            await send_message_complete(session_id, "", keyboard=keyboard)
        else:
