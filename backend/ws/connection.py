...
class MessageComplete(ServerMessage):
    ...
    keyboard: Optional[List[Dict[str, Any]]] = Field(default=None, description="快捷操作按钮")
...
async def send_message_complete(session_id: str, message_id: str, keyboard: Optional[List[Dict[str, Any]]] = None) -> int:
    """发送消息完成通知到会话

    Args:
        session_id: 会话 ID
        message_id: 消息 ID
        keyboard: 快捷操作按钮列表（可选）

    Returns:
        int: 成功发送的连接数
    """
    return await connection_manager.send_to_session(
        session_id, MessageComplete(message_id=message_id, keyboard=keyboard)
    )
