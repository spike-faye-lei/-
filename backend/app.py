...
async def lifespan(app: FastAPI):
    ...
    # 初始化 FaceAuth
    logger.info("Initializing FaceAuth...")
    try:
        from backend.modules.face_auth.pipeline import FacePipeline
        from backend.modules.face_auth.guard import FaceGuard, KnowledgeBaseGuard
        from backend.modules.face_auth.router import set_pipeline, set_guard
        from backend.modules.face_auth.frame_store import get_frame_store

        wiki_dir = shared["workspace"] / "wiki" / "concepts"
        face_pipeline = FacePipeline(data_dir=shared["workspace"] / "face_data")
        kb_guard = KnowledgeBaseGuard(wiki_dir=wiki_dir)

        # 创建 frame_capture 回调：从 FrameStore 异步拉取最新帧
        frame_store = get_frame_store()
        async def capture_frame():
            return await frame_store.latest()
        ...
        set_pipeline(face_pipeline)
        set_guard(face_guard)
        app.state.face_pipeline = face_pipeline
        app.state.face_guard = face_guard

        # 将 KnowledgeBaseGuard 注入 WikiService，使 KB 锁定生效
        try:
            from backend.modules.wiki.service import WikiService
            # WikiService 是单例模式，通过 get_wiki_service 获取并注入 guard
            from backend.api.wiki import get_wiki_service
            wiki_service = get_wiki_service()
            wiki_service.set_kb_guard(kb_guard)
            logger.info("KnowledgeBaseGuard wired to WikiService")
        except Exception as e:
            logger.warning(f"Failed to wire kb_guard to WikiService: {e}")

        logger.info("FaceAuth initialized")
    except Exception as e:
        logger.warning(f"FaceAuth init skipped (optional): {e}")
...
from backend.modules.face_auth.router import router as face_auth_router
...
if frontend_dist.exists():
    ...
    @app.get("/wiki/read/{slug}")
    async def spa_wiki_read(slug: str):
        return FileResponse(str(frontend_dist / "index.html"))
